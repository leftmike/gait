...
func TestReadFile(t *testing.T) {
	...
	out := callJSON(t, ag.readFile, readFileArgs{Path: path})
	want := "     1\talpha\n     2\tbeta\n     3\tgamma"
	if out != want {
	...
	out = callJSON(t, ag.readFile, readFileArgs{Path: path, Offset: 2, Limit: 1})
	if out != "     2\tbeta" {
		t.Fatalf("offset/limit = %q", out)
	}

	// Missing file is an error.
	callJSONErr(t, ag.readFile, readFileArgs{Path: filepath.Join(t.TempDir(), "nope")})
}
...
func TestReadFileEmpty(t *testing.T) {
	...
	if out := callJSON(t, ag.readFile, readFileArgs{Path: path}); out != "" {
